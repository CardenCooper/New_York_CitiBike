...
# Load main dataset
df = load_csv('reduced_data_to_plot_7.csv')
if df is None:
    st.stop()

# Optional top20 CSV
top20_csv = load_csv('top20.csv')  # can be None
...
myImage = load_image("CitiBike.jpg")
bikes = load_image("recs_page.jpg")
...
if page == "Intro page":
    ...
    st.markdown("Use the 'Aspect Selector' dropdown on the left to navigate.")
...
### Weather Component and Bike Usage ###
    ...
    for col in ['avgTemp', 'bike_rides_daily']:
        if col in df.columns:
            df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0)

    # Create daily temperature column
    if 'avgTemp' in df.columns:
        if df['avgTemp'].max() > 100:
            df['avgTemp_daily'] = df['avgTemp'].diff().fillna(df['avgTemp'])
        else:
            df['avgTemp_daily'] = df['avgTemp']

    # Create figure if required columns exist
    if all(col in df.columns for col in ['date', 'bike_rides_daily', 'avgTemp_daily']):
        ...
        st.warning("Insufficient data to display chart.")
...
### Interactive Map ###
    ...
    if os.path.exists(path_to_html):
        ...
        st.error(f"Map HTML file not found: {path_to_html}")
